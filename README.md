...
配置方式：在配置文件目录新建 **xml** 文档文件，该文件名称会显示在清理页面当作对应清理项名称，将自定义文件后缀填入该文件，使用空格 分隔文件后缀。
...
配置方式：在配置文件目录新建文档文件，文件名称随意但不得包含特殊符号、空格等。
...
这是一个优化功能集，包含磁盘优化（f2fs-GC）及Dexoat（软件预编译）功能
