...
[![THChat](https://img.shields.io/badge/YHchat-8068E2.svg?logo=icloud&logoColor=white)](https://yhfx.jwznb.com/share?key=yigOTedUjh62&ts=1747355950)
   ...
   # 🌍 ？
> **|** [**English**](https://github.com/FLYCOM-E/ClearBox/blob/main/README.md) **|** [**简体中文**](https://github.com/FLYCOM-E/ClearBox/blob/main/.docs/README_zh-CN.md) **|** [**正體中文**](https://github.com/FLYCOM-E/ClearBox/blob/main/.docs/README_zh-TW.md) **|** [**Русский**](https://github.com/FLYCOM-E/ClearBox/blob/main/.docs/README_ru-RU.md) **|** **한국어** **|**
...
구성 방식： 구성 파일 디렉터리에 **conf** 문서 파일을 새로 만듭니다. 파일 이름은 정리 페이지에 해당 정리 항목의 이름으로 표시됩니다. 사용자 정의 파일 확장자를 이 파일에 입력하고, 공백 또는 줄바꿈으로 구분합니다. 주석은 지원되지 않습니다.

파일 내에 **`@MAX=<size/B/K/M/G>`**, **`MIN=<size/B/K/M/G>`**을 입력하여 대상 파일 크기 범위를 지정할 수 있습니다. 후행 선언이 이전 선언을 덮어쓰므로 이 기능을 활용하여 원하는 규칙을 작성할 수 있습니다.
...
이 선언은 최대 **1 G**, 최소 **10 MB**의 **zip**, **7z** 파일만 분류하고, **iso**, **img**는 최대 크기 제한이 없음을 나타냅니다.

참고：**-1**은 제한 없음을 의미합니다. **MIN**은 **MAX**보다 클 수 없으며, 그렇지 않으면 **MIN**이 자동으로 무시됩니다. **MAX**, **MIN** 및 크기 단위는 **대소문자를 구분하지 않으며** (이미 눈치채셨겠죠 :⁠-⁠)），**소수점(실수)은 지원되지 않습니다**.

앱/터미널 정리 페이지는 구성 파일에 따라 동적으로 구축됩니다.

적용 범위： 파일 정리 및 이후에 설명할 파일 분류
