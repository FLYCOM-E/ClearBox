...
### [**CI建構**](https://github.com/FLYCOM-E/ClearBox/actions/workflows/building_modules.yml)
...
配置目錄： **`/data/adb/wipe_cache/FileConfigs`**

配置方式：在配置檔案目錄新建 **.conf** 文件檔案，該檔案名稱（不含副檔名）會顯示在清理頁面當作對應清理項名稱，將自訂檔案附檔名填入該檔案，使用空格或換行分隔，行首 # 即註解該行。

在檔案內填寫 **`@MAX 或 MIN = < 大小 / B 或 BYTE / K 或 KB / M 或 MB / G 或 GB >`** 可以指定目標檔案大小區間，覆蓋解析，後面的會覆蓋前面的設定，因此您可以利用此特性寫出您想要的規則
...
# 此宣告指明僅歸類最大 1G，最小 10MB 的 zip、7z
# 但 iso、img 則不限制最大大小
...
# max -1 表示無限制（max 在下次宣告前不再生效），但 min 仍然生效
...
另：**MIN** 不得大於 **MAX**，否則 **MIN** 自動失效，請在設定更大的 **MIN** 前重設 **MAX**。**MAX**、**MIN** 及大小單位是 **不區分大小寫** 的（您可能已經看出來了:⁠-⁠)），**暫不支援浮點（小數）**
...
配置目錄： **`/data/adb/wipe_cache/AppCleanRules`**
...
in=開始時間/結束時間（僅在X點到X點之間執行。這是一個可選參數）
post=通知標題/通知內容（傳送通知。這是一個可選參數，不建議在分鐘級任務加這個）
```
...
一鍵歸類內/多外部儲存檔案，規則與檔案清理共用，支援自訂歸類目錄



## 規則目錄綁定（重新導向）

配置目錄：**`/data/adb/wipe_cache/BindConfigs`**
...
註：請不要填寫 **絕對路徑**，而是 **相對內部儲存路徑**，否則會導致掛載失敗

當往 **<綁定目錄>** 內寫入/建立目錄/檔案時將被重新導向至 **<目標目錄>**

此功能暫無法在使用 **sdcardfs** 裝置上執行，但您無需擔心，**Android 12+** 基本已無裝置使用 **sdcardfs**
