...
### [**CI构建**](https://github.com/FLYCOM-E/ClearBox/actions/workflows/building_modules.yml)
...
配置方式：在配置文件目录新建 **.conf** 文档文件，该文件名称（不包含后缀）会显示在清理页面当作对应清理项名称，将自定义文件后缀填入该文件，使用空格或换行分隔，行首 # 即注释该行。

在文件内填写 **`@MAX 或 MIN = < 大小 / B 或 BYTE / K 或 KB / M 或 MB / G 或 GB >`** 可以指定目标文件大小区间，覆盖解析，后面的会覆盖前面的设定，因此您可以利用此特性写出您想要的规则
...
# 此声明指明仅归类最大 1G，最小 10MB 的 zip、7z
# 但 iso、img 则不限制最大大小
...
# max -1 表示无限制（max 在下次声明前不再生效），但 min 仍然生效
...
另：**MIN** 不得大于 **MAX**，否则 **MIN** 自动失效，请在设置更大的 **MIN** 前重设 **MAX**。**MAX**、**MIN** 及大小单位是 **非大小写敏感** 的（您可能已经看出来了:⁠-⁠)），**暂不支持浮点（小数）**
...
date=上次运行时间（时间戳数字）。这是一个可选参数，如果不填写则程序会自动追加为当前时间，设为 -1 可立即执行
run=要运行的程序及参数，支持 Shell 语法（交由 Shell 解析）
in=开始时间/结束时间（仅在X点到X点之间运行。这是一个可选参数）
post=通知标题/通知内容（发送通知。这是一个可选参数，不建议在分钟级任务加这个）
```

其中 **time**、**run** 为必填项，所有 **KEY** 可忽略大小写
实例：
...
一键归类内/多外部储存文件，规则与文件清理共用，支持自定义归类目录
