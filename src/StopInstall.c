...
#define MICRO_DIR "/mnt/expand/%s/app"
#define GET_SD_ID "ls /mnt/expand 2>/dev/null"
#define STOP_INSTALL "%s/busybox chattr +i %s 2>/dev/null"
#define RESET_INSTALL "%s/busybox chattr -i %s 2>/dev/null"
#define SETPROP_STOP "sed -i 's/stopinstall=0/stopinstall=1/g' %s/settings.prop"
#define SETPROP_RESET "sed -i 's/stopinstall=1/stopinstall=0/g' %s/settings.prop"
...
int main(int COMI, char * COM[])
{
    ...
    // Stop & Reset
    char command_data[strlen(bin_dir) + strlen(DATA_DIR) + 32];
    char command_micro[strlen(bin_dir) + strlen(micro_dir) + 32];
    ...
    char set_prop[strlen(work_dir) + 64];
    if (stop == 1)
    {
