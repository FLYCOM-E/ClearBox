...
long s_remove(char * path, int all)
{
    ...
    if (S_ISDIR(file_stat.st_mode))
    {
        ...
        {
            write_log(work_dir, "s_remove", L_OPEN_PATH_FAILED, path, strerror(errno));
        }
    }
    else
    {
...
int s_sed(char * file, char * target_line, char * text, int mode)
{
    ...
    if (file_fp == NULL)
    {
        write_log(work_dir, "s_sed", L_OPEN_FILE_FAILED, file, strerror(errno));
        if (tmp_fp) fclose(tmp_fp);
        return -1;
    }
    if (tmp_fp == NULL)
    {
        write_log(work_dir, "s_sed", L_OPEN_FILE_FAILED, tmp_file, strerror(errno));
        fclose(file_fp);
        return -1;
    }
